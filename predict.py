...
# Predict solar radiation and cloud coverage 5 minutes into the future
def predict_future_radiation_and_cloud_coverage(video_path, sensor_reading, model_path, frame_index, device):
    # Extract frames from the video
    frames,cloud = extract_frames(video_path)
    ...
    # Predict the solar radiation and cloud coverage 5 minutes into the future
    with torch.no_grad():
        prediction = model(processed_frame, sensor_tensor)
        # Extract the predictions for solar radiation and cloud coverage
        predicted_radiation = prediction[0, 0].item()  # First output (solar radiation)
        predicted_cloud_coverage = prediction[0, 1].item()  # Second output (cloud coverage)

    return predicted_radiation, predicted_cloud_coverage

# Example usage
if __name__ == "__main__":
    # Paths and device
    video_path = r"C:\Users\jldag\OneDrive\Desktop\Senior-Design\sky_images\1-25-24.mp4"
    ...
    # Choose a frame index (e.g., 500th frame)
    frame_index = 500

    # Predict future solar radiation and cloud coverage
    predicted_radiation, predicted_cloud_coverage = predict_future_radiation_and_cloud_coverage(
        video_path, current_sensor_reading, model_path, frame_index, device
    )
    print(f"Predicted solar radiation in 5 minutes: {predicted_radiation:.2f}")
    print(f"Predicted cloud coverage in 5 minutes: {predicted_cloud_coverage:.2f}%")